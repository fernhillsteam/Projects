...
# Form implementation generated from reading ui file 'calyxtract_vanilla_v1.2.ui'
#
...
class MainGUI_thread(Thread):
    ...
    def run(self):
        global SPEEDSERIAL  #Use global variable TEMPSERIAL
        ...
        GUIOBJECT = Ui_MainWindow()  # Create a GUI object
        GUIOBJECT.setupUi(MainWindow)  # Setup the main window in my
        # Gui object
        ...
        sys.exit(app.exec_())  # Exit the program when the main
        # windows is closed.
...
class Update_thread(Thread):
    ...
    def run(self):
        global GUIOBJECT  #Use global variable GUIOBJECT
        global SPEEDSERIAL #Use global variable SPEEDSERIAL
        global ser # open serial port
        print(ser.is_open) #Print and check if the port is open
        print(ser.name)    # Print the name of the port	zzz
        while 1: # For always
            line = ser.readline()# Read the line on serial port
            print(line.decode()) # Print the line from the serial port
            line = line.decode().replace('cs', '')
