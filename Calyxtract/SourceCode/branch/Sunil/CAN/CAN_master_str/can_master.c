...
volatile uint32_t g_ui32RXMsgCount2 = 0;
volatile uint32_t g_ui32TXMsgCount2 = 0;
...
uint8_t g_ui8RXMsgData[50];

/* slave 2 */

uint8_t g_ui8TXMsgData2[8];
uint8_t g_ui8RXMsgData2[50];
...
//            // Clear the message object interrupt.
//            //
//            CANIntClear(CAN1_BASE, RXOBJECT2);
//
//            //
//            // Increment a counter to keep track of how many messages have been
//            // received.  In a real application this could be used to set flags to
...
//            // Clear the message object interrupt.
//            //
//            CANIntClear(CAN1_BASE, TXOBJECT2);
//
//            //
//            // Increment a counter to keep track of how many messages have been
//            // transmitted. In a real application this could be used to set
...
{
    /*
     * Setup CAN1 to both send and receive at 500kps, interrupt enable
     * Pin Configure: PE4 - RX and PE5 - TX
     */
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOB); // Enable Peripheral on Port E
    GPIOPinConfigure(GPIO_PB0_CAN1RX); // Configure PE4 as CAN - RX
    GPIOPinConfigure(GPIO_PB1_CAN1TX); // Configure PE5 as CAN - TX
    GPIOPinTypeCAN(GPIO_PORTB_BASE, GPIO_PIN_0 | GPIO_PIN_1);
    SysCtlPeripheralEnable(SYSCTL_PERIPH_CAN1); // Enable Peripheral - CAN 0 Module
    ...
    //
    g_sCAN1RxMessage.ui32MsgID = CAN1RXID;  // 0
    ...
    g_sCAN1TxMessage.ui32MsgID = CAN1TXID;      // 2
    ...
    CANMessageSet(CAN1_BASE, TXOBJECT, &g_sCAN1TxMessage, MSG_OBJ_TYPE_TX); //Send Message

    /* slave 2 Tx object */

//    g_sCAN1TxMessage.ui32MsgID = CAN1TXID2;     // Txid=3
//    g_sCAN1TxMessage.ui32MsgIDMask = 0;
//    g_sCAN1TxMessage.ui32Flags = MSG_OBJ_TX_INT_ENABLE;
//    g_sCAN1TxMessage.ui32MsgLen = sizeof(g_ui8TXMsgData);
//    g_sCAN1TxMessage.pui8MsgData = (uint8_t *)&g_ui8TXMsgData2;
//
//    CANMessageSet(CAN1_BASE, TXOBJECT2, &g_sCAN1TxMessage, MSG_OBJ_TYPE_TX); //Send Message
}
        ...
        if(g_bRXFlag)
                {
                    ...
                    CANMessageGet(CAN1_BASE, RXOBJECT, &g_sCAN1RxMessage, 0);    // Read the message from the CAN.

                    g_bRXFlag = 0; //Clear the pending message flag so that the interrupt handler can set it again when the next message arrives.

                    if(g_sCAN1RxMessage.ui32Flags & MSG_OBJ_DATA_LOST)  //Checking for lost message
                     //   UARTprintf("\nCAN message loss detected\n");


                    UARTprintf("Message ID: 0x%03x Message Data:",g_sCAN1RxMessage.ui32MsgID);
                    UARTprintf("%c %c %c %c %c %c %c %c\n\r"
                            ,g_ui8RXMsgData[0],g_ui8RXMsgData[1],g_ui8RXMsgData[2],g_ui8RXMsgData[3]
                            ,g_ui8RXMsgData[4],g_ui8RXMsgData[5],g_ui8RXMsgData[6],g_ui8RXMsgData[7] );


                 //   UARTprintf("\nsize of data %d\n",sizeof(g_ui8RXMsgData));
                }
