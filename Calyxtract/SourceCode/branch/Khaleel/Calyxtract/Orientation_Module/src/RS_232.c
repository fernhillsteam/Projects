...
switch (select)
{
    case uart0: //UART0 initialization for debugging
    UART0_init();

    break;

    case uart2: //UART2 initialization for board 2 board communication
    UART2_init();
    break;

    case both: //Both UART0 and UART2 is initialized for debugging and board2board communication.
    UART0_init();
    UART2_init();
    break;
