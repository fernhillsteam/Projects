...
#define SCCB0_BASE GPIO_PORTB_BASE
#define SCCB0_SCL GPIO_PIN_4
#define SCCB0_SDA GPIO_PIN_5
...
//    while(!SysCtlPeripheralReady(SYSCTL_PERIPH_GPIOE));
    ...
    GPIOPinWrite(GPIO_PORTB_BASE,GPIO_PIN_4,GPIO_PIN_4);//输出高
    GPIOPinWrite(GPIO_PORTB_BASE,GPIO_PIN_5,GPIO_PIN_5);//输出高
    GPIOPinTypeGPIOOutput(SCCB0_BASE, SCCB0_SDA);//SDA Output
...
uint8_t SCCB0_Read_Reg(uint8_t reg)
{
    ...
    SCCB0_Write_Byte(0x42);//SCCB0_Write_Byte(0x43);
    Delay_us(100);
...
uint8_t SCCB0_Read_ID(uint8_t reg)
{
    uint8_t val=0;
    SCCB0_Start();
    //SCCB0_Write_Byte(SCCB0_ID);
